        ...
        <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
          <div className="flex w-2/5"> 
            ...
            <div className="flex flex-col justify-between ml-4 flex-grow">
              ...
              <button className="font-semibold hover:text-red-500 text-gray-500 text-xs" onClick={()=>dispatch(removeFromCart({id:item.id}))}>Remove</button>
            </div>
          </div>
          ...
          <span className="text-center w-1/5 font-semibold text-sm">{(item.price).toFixed(2)} { item.currencyCode==="TRY"&&"₺"}</span>
          <span className="text-center w-1/5 font-semibold text-sm">{(item.price*item.quantity).toFixed(2) } { item.currencyCode==="TRY"&&"₺"}</span>
        </div>
    )
}