...
export default function CartItem({ item }: Item) {
  ...
  useEffect(() => {
    //carttan gelen id ile kitapları getir
    axios
      .get(
        `${process.env.NEXT_PUBLIC_SINGLE_BOOK_API}${item.id}?&key=${process.env.NEXT_PUBLIC_API_KEY}`
      )
      .then((res) => {
    ...
    <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-0 w-2/5">
        ...
        <div className="flex flex-col justify-between sm:ml-4 flex-grow">
          ...
          <button
            className="font-semibold hover:text-red-500 text-gray-500 text-xs text-left mt-3 sm:mt-0"
      ...
      <span className="text-center w-2/5 sm:w-1/5 font-semibold text-sm">
        {item.price.toFixed(2)} {item.currencyCode === "TRY" && "₺"}
      </span>
      <span className="text-center w-1/5 font-semibold text-sm sm:block hidden">
