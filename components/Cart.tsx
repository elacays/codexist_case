...
export default function Cart() {
  ...
  return (
    ...
    <div className="flex shadow-md my-10">
      <div className="w-3/4 bg-white px-10 py-10">
        ...
        {/* <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
          ...
          <button onClick={()=>dispatch(reduceQuantity({id:1}))}>
            ...
            <input className="mx-2 border text-center w-8" type="text" disabled value={cartItems[1].quantity}/>
          ...
          <span className="text-center w-1/5 font-semibold text-sm">${(cartItems[1].price).toFixed(2)}</span>
          <span className="text-center w-1/5 font-semibold text-sm">${(cartItems[1].price*cartItems[1].quantity).toFixed(2) }</span>
        </div> */}
      ...
      <div id="summary" className="w-1/4 px-8 py-10 bg-gray-800 text-white">
        <h1 className="font-semibold text-2xl border-b pb-8">Order Summary</h1>
        <div className="flex justify-between mt-10 mb-5">
          <span className="font-semibold text-sm uppercase">Items 3</span>
          <span className="font-semibold text-sm">{total.toFixed(2)} {cartItems[0]&&cartItems[0].currencyCode==="TRY"&&"₺"||"$"}</span>
        </div>
        <div>
        ...
        <div className="border-t mt-8">
          <div className="flex font-semibold justify-between py-6 text-sm uppercase">
            ...
            <span>{(total!==0? (total+10):0).toFixed(2)} { cartItems[0]&&cartItems[0].currencyCode==="TRY"&&"₺"||"$"}</span>
          </div>
